...
namespace TheWorld.Models
{
    public class WorldContextSeedData
    {
        ...
        {
            if (await _userManager.FindByEmailAsync("rdow@theworld.com") == null)
            {
                var user = new WorldUser();
                user.UserName = "russelldow";
                user.Email = "rdow@theworld.com";

                await _userManager.CreateAsync(user, "P@$$w0rd");
            }

            //If there aren't any trips in the database, seed it with some sample data
